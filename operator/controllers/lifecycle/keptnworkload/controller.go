...
func (r *KeptnWorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if errors.IsNotFound(err) {
		...
		controllercommon.RecordEvent(r.Recorder, apicommon.PhaseCreateWorklodInstance, "Normal", workloadVersion, "WorkloadVersionCreated", "created KeptnWorkloadVersion ", workloadVersion.Spec.Version)
