...
func (in *AnalysisStatus) DeepCopyInto(out *AnalysisStatus) {
	*out = *in
	in.Timeframe.DeepCopyInto(&out.Timeframe)
	if in.StoredValues != nil {
		in, out := &in.StoredValues, &out.StoredValues
		*out = make(map[string]ProviderResult, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
...
func (in *KeptnMetricStatus) DeepCopyInto(out *KeptnMetricStatus) {
	*out = *in
	...
	in.LastUpdated.DeepCopyInto(&out.LastUpdated)
	if in.IntervalResults != nil {
		in, out := &in.IntervalResults, &out.IntervalResults
		*out = make([]IntervalResult, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
