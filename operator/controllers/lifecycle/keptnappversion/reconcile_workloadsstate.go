...
func (r *KeptnAppVersionReconciler) reconcileWorkloads(ctx context.Context, appVersion *klcv1alpha3.KeptnAppVersion) (apicommon.KeptnState, error) {
	...
	for _, w := range appVersion.Spec.Workloads {
		...
		workload, err := r.getWorkloadVersion(ctx, getWorkloadVersionName(appVersion.Namespace, appVersion.Spec.AppName, w.Name, w.Version))
		if err != nil && errors.IsNotFound(err) {
			controllercommon.RecordEvent(r.Recorder, phase, "Warning", appVersion, "NotFound", "workloadVersion not found", appVersion.GetVersion())
...
func (r *KeptnAppVersionReconciler) getWorkloadVersion(ctx context.Context, workload types.NamespacedName) (klcv1alpha3.KeptnWorkloadVersion, error) {
	workloadVersion := &klcv1alpha3.KeptnWorkloadVersion{}
	err := r.Get(ctx, workload, workloadVersion)
	return *workloadVersion, err
}

func getWorkloadVersionName(namespace string, appName string, workloadName string, version string) types.NamespacedName {
