...
at runtime the metrics operator will try to substitute everything in `{{.VariableName }}` format with a key-value pair in the
Analysis resource,
...
check [here](https://github.com/keptn/lifecycle-toolkit/tree/main/test/integration/analysis-controller-multiple-providers)
