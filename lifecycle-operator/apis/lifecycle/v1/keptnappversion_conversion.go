...
// ConvertTo converts the src v1.KeptnAppVersion to the hub version (v1beta1.KeptnAppVersion)
//
//nolint:gocyclo
func (src *KeptnAppVersion) ConvertTo(dstRaw conversion.Hub) error {
	...
	dst.Status.PromotionStatus = v1beta1common.KeptnState(src.Status.PromotionStatus)
	...
	for _, item := range src.Status.PromotionTaskStatus {
		dst.Status.PromotionTaskStatus = append(dst.Status.PromotionTaskStatus, v1beta1.ItemStatus{
			DefinitionName: item.DefinitionName,
			Status:         v1beta1common.KeptnState(item.Status),
			Name:           item.Name,
			StartTime:      item.StartTime,
...
// ConvertFrom converts from the hub version (v1beta1.KeptnAppVersion) to this version (v1.KeptnAppVersion)
//
//nolint:gocyclo
